...
# calculates child's age in days and months as integers and adds them.

# The script requires 2 data files: data_CDI.csv, data_PQ.csv 
# Those files were generated by 01_kidswords_dataprep.R
# in order to anonymize the original data files and
# remove records from the website's data entry test phase
# and Learning to Talk project. 

# The script also requires a 3rd file: tb_CDI_word_features.csv
...
# rename two variables and define them as dates 
...
CDI <- mutate(CDI, cagedays = session_date - DOB)

# convert child's age in days to an integer
CDI$cagedays <- as.integer(CDI$cagedays)

# calculate child's age in months + days, as a period
CDI$cagemosdays <- as.period(interval(CDI$DOB, CDI$session_date), unit = "months")

# extract month as an integer for use in statistical analyses
CDI$cagemos <- month(CDI$cagemosdays)
...
CDI <- select(CDI, PID, DOB, session_date, session, cagemosdays, cagemos, 
              cagedays, CDI_item, measurement)
CDI_wide <- select(CDI_wide, PID, DOB, session_date, session, cagemosdays, 
                   cagemos, cagedays)
...
# merge CDI and PQ dataframes
# data <- merge(CDI, PQ, by = "PID")
# data <- merge(data, co_m, by = "PID")
