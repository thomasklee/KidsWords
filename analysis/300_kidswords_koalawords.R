...
p <- ggplot(data = AoA_table, mapping = aes(x = camos, y = prop_said))
...
p + theme_light() + 
  ...
  labs (title = "Age of acquisition of Koala words based on NZ CDI:WS (N = 2,581) \n") +
  facet_wrap(~CDI_item)

# save plot as PDF
ggsave(filename = "figures/koala9.pdf")
